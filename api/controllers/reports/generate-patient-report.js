  ...
  fn: async function (inputs, exits) {
    try {
      ...
      if (inputs.gender && inputs.gender.length > 0) {
        const genders = inputs.gender.map((g) => `'${g}'`).join(",");
        gender_filter = ` AND t1.gender IN (${genders}) `;
      }
      ...
      var patient_summary = await sails.sendNativeQuery(patient_sql);
      patient_summary = patient_summary.rows;
